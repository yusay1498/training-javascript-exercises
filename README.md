...
4. 実装できたら、以下のいずれかの方法で確認します。
   - **自動採点があるもの**: ターミナルで `node exercises/番号-名前/check.js`（または `cd exercises/番号-名前 && node check.js`）を実行する
...
`exercises/`の演習が一通り終わったら、任意（ボーナス）の腕試しとして、
簡単なアプリやゲームをいくつか用意しています。

**[→ 追加チャレンジ一覧（challenges/README.md）](challenges/README.md)**

- Hit and Blow、じゃんけんゲーム、ToDoリストアプリ、神経衰弱（Memory Match）など
- これまでの演習で学んだ関数・書き方を組み合わせて、動くものを完成させる体験ができます
...
初めて開発環境を構築する方向けに、Git・Node.js（nvm）・エディタのインストールから
リポジトリの取得までを詳しく解説した手順書を用意しています。

**[→ セットアップ手順（docs/SETUP.md）](docs/SETUP.md)**

- Windowsの方は、WSL（Windows Subsystem for Linux）上での開発を推奨しています。
- macOSの方も、手順書内に沿ってセットアップを進めてください。
